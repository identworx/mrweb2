...
export default function HomepageCollections({ section, collections, icons = {}, locale = "de" }: Props) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">
          ...
          >
            ...
            <div className="flex flex-col items-start justify-center flex-1 p-5 pt-6">
              <p className="font-accent text-text-muted text-[10px] tracking-[0.25em] uppercase mb-3">
                {locale === "en" ? "All Colour Worlds" : "Alle Farbwelten"}
              </p>
              <h3 className="font-heading text-anthracite text-lg md:text-xl font-bold tracking-tight leading-snug mb-3">
                {locale === "en" ? "View all collections" : "Alle Kollektionen ansehen"}
              </h3>
              <p className="font-body text-text-gray text-[13px] leading-[1.75]">
                {locale === "en" ? "Discover all colour worlds and materials at a glance." : "Entdecken Sie alle Farbwelten und Materialien im Überblick."}
              </p>
            </div>
              ...
              <div className="flex items-center justify-end mt-4 pt-3 border-t border-black/[0.05]">
                <span className="flex items-center gap-1.5 text-anthracite group-hover:text-pumpkin transition-colors duration-500">
                  <span className="font-heading text-[10px] font-semibold uppercase tracking-[0.14em]">
                    {locale === "en" ? "View overview" : "Übersicht öffnen"}
                  </span>
