...
export default function MaterialQualityBox({ icons = {}, locale = "de" }: { icons?: Record<string, ResolvedIcon>; locale?: Locale }) {
  return (
    <div className="bg-white border border-light-gray p-6 md:p-8">
      <div className="flex items-center gap-4 mb-5">
        ...
        <p className="font-accent text-text-muted text-xs tracking-[0.3em] uppercase">
          {locale === "en" ? "Four Qualities" : "Vier Qualitäten"}
        </p>
      </div>

      <h3 className="font-heading text-anthracite text-lg md:text-xl font-bold mb-2">
        {locale === "en" ? "Which fabric for outdoor furniture?" : "Welcher Stoff für Outdoormöbel?"}
      </h3>
      ...
      <Link
        href={localizedHref("/materialien", locale)}
      ...
      >
        <span className="font-heading text-[12px] font-semibold uppercase tracking-[0.12em]">
          {locale === "en" ? "All qualities in detail" : "Alle Qualitäten im Detail"}
        </span>
