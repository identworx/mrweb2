...
async function main() {
  ...
  const pages = [
    ...
    { slug: "kollektionen", title: "Kollektionen", type: "COLLECTION_INDEX" as const, eyebrow: "Saison 2027 · Outdoor Living", headline: "Sieben Farbwelten.", introText: "Sieben kuratierte Farbwelten für den Außenbereich — jede Kollektion erzählt ihre eigene Geschichte aus Farbe, Material und Stimmung.", seoTitle: "Kollektionen 2027 | Mosaroma", seoDescription: "Entdecken Sie die 7 Kollektionen von MOSAROMA — kuratierte Farbwelten für den Außenbereich." },
  ...
  // ---------------------------------------------------------------------------
  // 3c. PageSections for homepage
  // ---------------------------------------------------------------------------
  const homePage = await prisma.page.findUnique({ where: { slug: "home" } });

  if (homePage) {
    const existingHomeSections = await prisma.pageSection.count({ where: { pageId: homePage.id } });
    if (existingHomeSections === 0) {
      ...
      console.log("✔ PageSections: home (7 sections)");
    } else {
      console.log(`✔ PageSections: home already has ${existingHomeSections} sections, skipping`);
    }
  }
